      ...
      {options.map((o) => (
        <button
          ...
          className={`relative px-3.5 py-1.5 text-[13px] rounded-lg transition-colors cursor-pointer ${
            value === o ? "text-on-accent font-semibold" : "text-mute hover:text-ink2"
          }`}
        >
          {value === o && (
            <motion.div
...
export function Button({
  ...
  const styles = {
    primary: "bg-accent hover:bg-accent2 text-on-accent",
    ghost: "bg-card2 hover:bg-[#232e3d] text-ink2",
...
export function TextInput({
  ...
  return (
    <input
      ...
      spellCheck={false}
      className={`w-full rounded-xl bg-card2 px-3.5 py-2.5 text-[13px] text-ink outline-none placeholder:text-mute focus:ring-1 focus:ring-accent transition-shadow ${className}`}
    />
  );
}
