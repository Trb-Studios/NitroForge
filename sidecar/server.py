"""
FPSBooster sidecar: a local-only JSON API over the existing `core` package,
...
Security model:
  ...
  * Every request must carry the X-FPSB-Token header the shell generated.
    Browsers can't attach custom headers cross-origin without passing CORS
    preflight, and we only ACK our own origins - so random websites can't
    drive this API, and other local processes would need the token.
...
from core import (bottleneck, config, game_scanner, logger as logmod,
                  network_utils, power_utils, process_utils, resolution_utils,
                  system_info)
...
class Services:
    ...
    def games(self, rescan=False) -> list[dict]:
        if self._games_cache is None or rescan:
            self._games_cache = [asdict(g) for g in
                                 game_scanner.scan_all(self.settings)]
            for g in self._games_cache:
                g["fso_disabled"] = (power_utils.fullscreen_opt_disabled(g["exe"])
                                     if g["exe"] else False)
...
def h_game_launch(_q, body):
    game = game_scanner.Game(body["name"], body["exe"], body["source"],
                             body.get("install_dir", ""))
    ...
    if boost or res:
        proc = SVC.booster.boost_and_launch(game)
    else:
        proc = game_scanner.launch(game)
    if proc and SVC.fps.running:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle(self, method: str):
        if self.headers.get("X-FPSB-Token", "") != TOKEN:
        ...
        if method == "POST":
            ...
            if n:
                try:
                    ...
                    return
        try:
            self._reply(200, fn(parse_qs(url.query), body))
